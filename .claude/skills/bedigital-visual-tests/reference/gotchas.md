...
Adversarial missions create/delete/edit data. Two missions against the same live app + DB corrupt each other's assumptions, so review mode runs missions **sequentially** and calls `sandbox.sh reset` before each one. Never fan missions out in parallel against one sandbox. The default `reset` recreates the data services + their anonymous volumes (fresh empty DB), re-runs the recipe's `MIGRATE_CMD`/`SEED_CMD` to rebuild schema + seed, restarts the app **in place** so it drops any stale DB pool (the container is kept, so `SANDBOX_URL` survives), and re-polls `HEALTH_PATH` before returning. Wire `MIGRATE_CMD`/`SEED_CMD` for any DB-backed app — the DB image will not seed your app's data itself. `RESET_CMD` remains only as a full override (e.g. a DB on a **named** volume that `rm` won't wipe).
