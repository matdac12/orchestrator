...
7. **Commit everything the compose references** into the target repo — every bind-mount source (`schema.sql`, `seed.sql`, `supabase/*.sql`, `gateway.conf`, seed scripts), not just the recipe and compose. `up`/`onboard` build from a **committed-HEAD worktree** and overlay only `recipe.env`, `sandbox.compose.yml`, `Dockerfile.*`, `.dockerignore` uncommitted; anything else the compose mounts must exist at HEAD (why, and the failure it causes: `reference/onboarding.md` §2, and the repair table below). So: author, smoke-verify uncommitted (the overlaid files iterate fine), then commit the rest *before the real* `up`. Then hand off: *"onboarded — run `/esegui-test` to test."*
